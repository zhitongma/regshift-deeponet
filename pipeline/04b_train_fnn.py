#!/usr/bin/env python3
"""
Step 4b: 训练 FNN Baseline (标准全连接网络, 无算子结构)

与 04_train_m1.py 使用完全相同的数据、训练超参和评估流程,
唯一区别是模型架构从 DeepONet 换成了 FNN。
checkpoint 保存为 fnn_best.pt / fnn_final.pt。
"""
...
def train_epoch(model, loader, trunk, optimizer, device, w_h, w_c,
    ...
    n_batches = 0
    M = trunk.shape[0]
    nc = min(n_coords_train, M) if n_coords_train else M
    for branch_b, h_b, c_b in loader:
        ...
        if nc < M:
            idx = torch.randperm(M, device=device)[:nc]
            trunk_sub = trunk[idx]
            h_sub = h_b[:, idx]
            c_sub = c_b[:, idx]
        else:
        ...
        optimizer.zero_grad()
        pred = model(branch_b, trunk_sub)
        loss = data_loss(pred, h_sub, c_sub, w_h=w_h, w_c=w_c)
        loss.backward()
        optimizer.step()
...
def evaluate(model, dataset, trunk, device, w_h, w_c):
    ...
    for start in range(0, N, EVAL_BATCH):
        ...
        total_loss += data_loss(pred, h_all[start:end], c_all[start:end],
                                w_h=w_h, w_c=w_c).item() * (end - start)
...
def main():
    parser = argparse.ArgumentParser(description="Train FNN baseline")
    ...
    logger = configure_logger(__name__, paths.logs / "train_fnn.log")
    save_config_snapshot(cfg, cfg_path, paths, "train_fnn")
    ...
    n_coords_train = tcfg.get("n_coords_train", 1024)
    ...
    if time_stride and time_stride > 1:
        ...
        logger.info("FNN using SubsampledGridDataset: time_stride=%d", time_stride)
    else:
    ...
    full_trunk = (train_ds.full_trunk_inputs.float().to(device)
                  if hasattr(train_ds, 'full_trunk_inputs')
                  else val_ds.trunk_inputs.float().to(device))
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True,
                              num_workers=0, pin_memory=True)

    model = build_fnn_model(cfg["model"]).to(device)
    logger.info("FNN Baseline: %d parameters", model.num_params)

    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    pat_checks = patience_epochs_to_checks(tcfg["lr_patience"], val_interval)
    scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=tcfg["lr_factor"],
                                  patience=pat_checks, min_lr=tcfg["lr_min"])
    es_checks = patience_epochs_to_checks(tcfg["early_stopping_patience"], val_interval)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if should_validate(epoch, val_interval):
            val_loss, val_h_l2, val_c_l2 = evaluate(model, val_ds, full_trunk, device, w_h, w_c)
            ...
            if val_loss < best_val:
                best_val = val_loss
                best_epoch = epoch
                torch.save(model.state_dict(), paths.checkpoints / "fnn_best.pt")

            if epoch % (val_interval * 10) == 0 or epoch == total_epochs:
                logger.info(
                    "Epoch %d/%d  train=%.6f  val=%.6f  h_l2=%.5f  c_l2=%.5f  lr=%.1e  best@%d",
                    epoch, total_epochs, train_loss, val_loss, val_h_l2, val_c_l2,
                    optimizer.param_groups[0]["lr"], best_epoch,
                )

            no_improve = epochs_without_improvement(epoch, best_epoch)
            if no_improve >= tcfg["early_stopping_patience"]:
                logger.info("Early stopping at epoch %d (no improve for %d epochs)",
                            epoch, tcfg["early_stopping_patience"])
                break

    elapsed = time.time() - t0
    torch.save(model.state_dict(), paths.checkpoints / "fnn_final.pt")

    test_loss, test_h_l2, test_c_l2 = evaluate(model, test_ds, full_trunk, device, w_h, w_c)
    ...
    history_out = {
        **{k: [_to_py(x) for x in v] if isinstance(v, list) else v
    ...
    with open(paths.experiments / "fnn_history.json", "w") as f:
        json.dump(history_out, f, indent=2)

    save_manifest(paths, "train_fnn", {
        "checkpoints": {
            "best": str(paths.checkpoints / "fnn_best.pt"),
            "final": str(paths.checkpoints / "fnn_final.pt"),
        },
        "total_epochs": epoch,
        "best_epoch": best_epoch,
    ...
    logger.info("Done. Best epoch=%d, elapsed=%.1f min", best_epoch, elapsed / 60)
