#!/usr/bin/env python3
"""
Step 4c: 训练 FNO Baseline (Fourier Neural Operator)

与 04b_train_fnn.py 使用相同的数据和评估流程,
模型架构从 FNN 换成 2D FNO。
checkpoint 保存为 fno_best.pt / fno_final.pt。
"""
...
def main():
    ...
    logger = configure_logger(__name__, paths.logs / "train_fno.log")
    save_config_snapshot(cfg, cfg_path, paths, "train_fno")
    ...
    logger.info("FNO 2D: %d parameters", model.num_params)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if should_validate(epoch, val_interval):
            ...
            if val_loss < best_val:
                best_val = val_loss
                best_epoch = epoch
                torch.save(model.state_dict(), paths.checkpoints / "fno_best.pt")
    ...
    torch.save(model.state_dict(), paths.checkpoints / "fno_final.pt")
    ...
    with open(paths.experiments / "fno_history.json", "w") as f:
        json.dump(history_out, f, indent=2)

    save_manifest(paths, "train_fno", {
        "checkpoints": {
            "best": str(paths.checkpoints / "fno_best.pt"),
            "final": str(paths.checkpoints / "fno_final.pt"),
        },
        "total_epochs": epoch,
        "best_epoch": best_epoch,
